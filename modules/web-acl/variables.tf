...
variable "managed_rule_sets" {
  ...
  validation {
    ...
    error_message = "managed_rule_sets: rule_group_name must be a valid AWS managed rule set name. Valid names include: AWSManagedRulesCommonRuleSet, AWSManagedRulesKnownBadInputsRuleSet, AWSManagedRulesLinuxRuleSet, AWSManagedRulesSQLiRuleSet, AWSManagedRulesAmazonIpReputationList, AWSManagedRulesAnonymousIpList, AWSManagedRulesATPRuleSet, AWSManagedRulesBotControlRuleSet, AWSManagedRulesUnixRuleSet, AWSManagedRulesWordPressRuleSet, AWSManagedRulesAdminProtectionRuleSet, AWSManagedRulesWindowsRuleSet, AWSManagedRulesPHPRuleSet."
  }
}
...
variable "logging" {
  ...
  validation {
    condition = var.logging == null ? true : (
      ...
      || can(regex("^aws-waf-logs-[a-zA-Z0-9][a-zA-Z0-9-]*$", var.logging.cloudwatch_log_group_name))
    )
    error_message = "CloudWatch log group name must be null or start with 'aws-waf-logs-' and be valid."
  }
}
